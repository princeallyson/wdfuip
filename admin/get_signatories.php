...
if (mysqli_num_rows($approved) > 0) {
    while ($row = mysqli_fetch_assoc($approved)) {
        ...
        <div class="modal-body">
            <button type="button" class="btn btn-primary mb-4 float-right">Download</button>
            <div class="row">';
        break;
    }
}
...
if (mysqli_num_rows($query) > 0) {
    ...
    <input type="file" id="fileInput" name="fileInput" style="display: none;" accept="image/*">
                  <button type="button" class="w-100 btn btn-primary view-button mb-2" onclick="handleFileInput('.$document_id.', '.$user_id.', '.$signatory_id.', \'Approved\')">Accept</button>
                  </li>
              <li class="list-group-item">
                <button class="w-100 btn btn-primary view-button mb-2" onclick="handleFileInput('.$document_id.', '.$user_id.', '.$signatory_id.', \'Rejected\')">Reject</button>
              </li>
              </form>';
} else if (!is_null($conversation)){
